...
#############################################################################################
@app.callback(
    Output('tabs-alert-content', 'children'),
    [Input('tabs-alert-dashboard', 'value')]
)
...
#############################################################################################
julio_tab = dbc.Container(
    html.Div([
    html.H2('Malware and Threats Alerts Dashboard'),
    dcc.Tabs(id="tabs-alert-dashboard", value='malware-and-threat-alerts', children=[
        dcc.Tab(label='Malware and Threat Alerts', value='malware-and-threat-alerts'),
        dcc.Tab(label='Threat Monitoring', value='threat-monitoring')
    ]),
    html.Div(id='tabs-alert-content')
])
)
start_date = str(datetime.date(2025, 6, 1))
end_date = str(datetime.date(2025, 6, 30))
######################################################################
tabs = dbc.Tabs(
    [
        dbc.Tab(tab1_siem, label="SIEM Overview"),
        dbc.Tab(tab2_auth, label="Authentication Overview"),
        # dbc.Tab(label="Malware and Threat Alerts",value='malware-and-threat-alerts',children=[html.Div([
